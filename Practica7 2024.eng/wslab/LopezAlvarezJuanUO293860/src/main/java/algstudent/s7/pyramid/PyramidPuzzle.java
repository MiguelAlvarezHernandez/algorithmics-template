...
public class PyramidPuzzle extends BranchAndBound {	
	 ...
	 */
    public PyramidPuzzle(SquareBoard board) {
...
class SquareBoard extends Node {
    ...
    public void calculateHeuristicValue() {
    	...
    	if(prune()) {
    		...
    		for(int i = 0; i<n; i++) {
    			for(int j = 0; j<i+1; j++) {
    				if(board[i][j] == 0) counter++; //Count the * in the board
    			}
    		}
	...
	private boolean prune() {
		for(int i = 0; i<n-1; i++) {
			for(int j = 0; j<i+1; j++) { //Avoid checking the last row because it doesnt have anything below
				if(board[i][j] > 0 && board[i+1][j]>0 && board[i+1][j+1] > 0) {
					boolean valid = false;
					if(board[i][j] == board[i+1][j] + board[i+1][j+1]) {
						valid = true;
					}
					if(board[i][j] == Math.abs(board[i+1][j] - board[i+1][j+1])){
						valid = true;
					}
					if(!valid) return true; //Prune
					}
			}
		}
		return false; // Dont prune
	}
	...
	public ArrayList<Node> expand() {
		...
		while(board[row][column] != 0) {
			if(column > 0) {
				...
				row--;
				column = row; //move to the last column of that row, which is in fact the same number as the row
			}
		}
		for(int k = 1; k<10; k++) {
	 ...
	 * @param parentID
	 */
    public SquareBoard(int[][] board, int depth, UUID parentID, int row, int column) {
    	this.board = board;
    	this.depth = depth;
