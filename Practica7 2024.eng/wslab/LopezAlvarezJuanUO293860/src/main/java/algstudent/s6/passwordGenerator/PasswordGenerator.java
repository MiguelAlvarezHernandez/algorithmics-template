package algstudent.s6.passwordGenerator;
...
public class PasswordGenerator {	
	...
	private void backtracking(int level) {
		if(level == numberOfTotalCharacters) { //It is a solution
			found = true;
			passwords.add(String.valueOf(password));
		}
		else { //It is not a solution
			Character[] shuffle;
			if(level < numberOfTotalCharacters - numberOfNonLettersEnds) { //should be alphabets
				Collections.shuffle(lettersList);
				shuffle = lettersList.toArray(new Character[lettersList.size()]);
			}
			else { //should be punctuation marks or numbers
			...
			for(int i=0; i<shuffle.length; i++) {
				if(!found) {
					...
					if(valid(level, character)) {
						...
						password[level]='?'; //clean the wrong path (not really necessary)
					}
				}
			}
		}
	...
	private boolean valid(int level, char letter) {
		...
		// Only some consontant pairs are allowed
	...
	private boolean isConsonant(char letter) {
		return "qwrtypsdfghjklzxcvbnm".indexOf(letter) != -1;
	}
