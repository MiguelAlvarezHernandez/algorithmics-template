...
public class RoadsInCity {
	...
	public long calculate(int startX, int startY, int endX, int endY) {
		...
		if (checkPositions()) { //O(1)
			...
			//3-Calculate number of paths each square is the number of ways I can reach that position
	...
	private boolean checkPositions() {
		if(startX < 0 || startX >= rows) return false;
		if(startY < 0 || startY >= columns) return false;
		if(endX < 0 || endX >= rows) return false;
		if(endY < 0 || endY >= columns) return false;
	 ...
	 * We should take into account that we cannot put 1 if it is a barrier (if it is -1)
	 */
	...
	private void calculateNumberOfPaths(){
		for(int i = startX; i>=endX; i--) { //From bottom to top
			for(int j = startY; j<=endY; j++) { //From left to right
				if(map[i][j] != -1) {
					if(i+1 < rows) {
						if(map[i+1][j] != -1)
							map[i][j] += map[i+1][j];
